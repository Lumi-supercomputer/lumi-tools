...
function print_help()
    ...
    print( 
        '\nlumi-workspaces: Print information about current quota and allocation information.\n\n' ..
...
local user_path = '/var/lib/user_info/users/'
local project_path = '/var/lib/project_info/users/'

local user_postfix = user .. '/' .. user .. '.json'
user_file = user_path .. user_postfix

-- print( 'Now attempting to read information from ' user_file )
fh = io.open( user_file, 'r' )
if fh == nil then 
    io.stderr:write( 'ERROR: Fail to read data about your userid, cannot continue.\n\n' )
    os.exit( 1 )
end
...
if project_list ~=  nil and #project_list > 0 then
    ...
    for _,project in ipairs( project_list ) do
        ...
        --
        -- Try to open the file with project data
        --

        local project_postfix = project .. '/' .. project .. '.json'
        local project_file = project_path .. project_postfix
        -- print( 'Now attempting to read information from ' .. project_file )
        local fh = io.open( project_file, 'r' )
        if fh == nil then 
            io.stderr:write( 'ERROR: Failed to read the data for project ' .. project .. ', cannot continue.\n\n' )
            os.exit( 1 )
        end
        local project_info_str = fh:read( '*all' )
        fh:close()
        ...
        if project_info['storage_quotas']['directories'] == nil or project_info['storage_quotas']['directories']['projappl'] == nil then
            ...
            -- Figure out on which file system the project is hosted. This is not in the json file.
        ...
        --
        -- Print information on the allocation
        --
...
else -- if project_list ~=  nil and #project_list > 0
